...
class FileCarvingTool(QMainWindow):
    ...
    def display_metadata(self):
        if not self.carved_data:
            self.status_bar.showMessage("No carved data available to extract metadata.")
            return

        try:
            magic_mime = magic.Magic(mime=True)
            file_type = magic_mime.from_buffer(self.carved_data)
            metadata = {}
            if file_type.startswith("image/"):
                try:
                    ...
                    with io.BytesIO(self.carved_data) as image_data:
                    ...
                    self.status_bar.showMessage("PIL library not found for image metadata extraction.")
            elif file_type.startswith("application/pdf"):
                try:
                    from PyPDF2 import PdfReader

                    with open(self.export_carved_file, 'rb') as pdf_file:  # Assuming carved data is exported first
                        pdf_reader = PdfReader(pdf_file)
                        metadata["Author"] = pdf_reader.metadata.get('/Author')
                        metadata["Title"] = pdf_reader.metadata.get('/Title')
                        metadata["Subject"] = pdf_reader.metadata.get('/Subject')
                except ImportError:
                    self.status_bar.showMessage("PyPDF2 library not found for PDF metadata extraction.")
            else:
                metadata["File Type"] = file_type.split("/")[1]

            metadata_text = "Metadata:\n"
            for key, value in metadata.items():
                metadata_text += f"{key}: {value}\n"
            self.text_browser.append(metadata_text)
            self.status_bar.showMessage("Metadata extracted and displayed.")

        except Exception as e:
            self.status_bar.showMessage(f"Error extracting metadata: {e}")
